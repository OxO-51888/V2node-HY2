...
func selectServerTxForCongestion(clientRx uint64, bandwidth BandwidthConfig) uint64 {
	if bandwidth.ForceServerBandwidth && bandwidth.MaxTx > 0 {
		return bandwidth.MaxTx
	}
	if bandwidth.MaxTx > 0 && clientRx > bandwidth.MaxTx {
		return bandwidth.MaxTx
	}
	return clientRx
}
...
func (h *h3sHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodPost && r.Host == protocol.URLHost && r.URL.Path == protocol.URLPath {
		...
		actualTx := selectServerTxForCongestion(authReq.Rx, h.config.BandwidthConfig)
